...
const STYLE_DETAILS: Record<string, StyleNarrative> = {
  ...
  scandinavian: {
    ...
    faqs: [
      ...
      {
        question: "Does Scandinavian style work in warm Indian cities?",
        answer:
  ...
  bohemian: {
    intro:
      "Bohemian interior design is ideal for people who want their rooms to feel expressive, layered, and full of personality. Instead of strict rules, boho spaces grow from collected objects, handcrafted textures, meaningful colors, and relaxed furniture layouts. That makes the style especially attractive in India, where textiles, artisanal materials, plants, and cultural color already play a strong role in home life. A bohemian room does not need expensive uniform furniture to feel finished. It needs balance between color, comfort, and individuality. AI room redesign is useful here because boho styling can be hard to imagine from swatches alone. Seeing your actual room transformed with warmer tones, layered fabrics, cane furniture, and plant life makes it easier to decide how bold or subtle you want the final outcome to be.",
    whatIs:
      "Bohemian design values freedom over perfection. It mixes textures, patterns, and influences in a way that feels curated by life rather than staged by a catalog. You may see rattan furniture next to upholstered seating, patterned cushions over plain bedding, woven pendants above wooden tables, and art collected from travel, craft fairs, or family homes. The room should feel lived in, not overplanned. That said, good bohemian design still has structure. Repeating a color family, keeping circulation comfortable, and using a few anchor pieces prevents the room from becoming messy. In Indian homes, boho design often feels naturally compatible because materials like cane, jute, block prints, ceramics, brass, and terracotta already sit comfortably within the local design language.",
    palette:
    ...
    faqs: [
      ...
      {
        question: "Which Indian materials suit bohemian interiors best?",
        answer:
...
export async function generateMetadata({ params }: StylePageProps): Promise<Metadata> {
  ...
  return {
    title: {
      absolute: `${styleData.name} Room Design — AI Interior Redesign ₹749 | AltorLab`,
    },
    description: `${styleData.name} interior design ideas for Indian homes. Explore colors, furniture tips, and AI-powered room redesign from ₹749 with AltorLab.`,
    openGraph: {
      title: `${styleData.name} Room Design — AI Interior Redesign ₹749 | AltorLab`,
      description: `Plan a ${styleData.name.toLowerCase()} room with AI. See palette, furniture, and India-specific design tips before you redesign.`,
      url: `https://app.altorlab.org/styles/${styleData.slug}`,
    },
    twitter: {
      card: "summary_large_image",
      title: `${styleData.name} Room Design — AI Interior Redesign ₹749 | AltorLab`,
      description: `See how ${styleData.name.toLowerCase()} interior design can transform your room with AI from ₹749.`,
    },
  };
}
  ...
  return (
    <main className="min-h-screen bg-slate-900 text-slate-300">
      ...
      <div className="mx-auto max-w-4xl space-y-12 px-4 py-14 sm:px-6 lg:px-8">
        ...
        <section>
          <h2 className="mb-4 text-2xl font-bold text-white">India-specific tips for {styleData.name.toLowerCase()} interiors</h2>
        ...
        <section className="rounded-3xl border border-indigo-500/20 bg-indigo-500/10 p-8">
          ...
          <p className="mb-6 leading-8 text-indigo-100">
            Upload your current room photo, choose {styleData.name}, and get an AI redesign preview from ₹749. It is the fastest way to validate whether this style fits your space, lighting, and furniture layout.
          </p>
